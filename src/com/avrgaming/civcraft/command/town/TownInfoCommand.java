...
public class TownInfoCommand extends CommandBase {
	...
	public void init() {
		...
		commands.put("safety", "Shows information about this town's safety.");
	...
	public void safety_cmd() throws CivException {
		Town town = getSelectedTown();
		
		CivMessage.sendHeading(sender, "Safety Info");
		ArrayList<String> out = new ArrayList<String>();

		out.add(CivMessage.buildSmallTitle("Safety Sources"));
		AttrSource safetySources = town.getSafety();

		DecimalFormat df = new DecimalFormat();
		df.applyPattern("###,###");
		for (String source : safetySources.sources.keySet()) {
			Double value = safetySources.sources.get(source);
			out.add(CivColor.Green+source+": "+CivColor.LightGreen+df.format(value));
		}
		out.add(CivColor.LightPurple+"Total: "+CivColor.LightGreen+df.format(safetySources.total));

		
		out.add(CivMessage.buildSmallTitle("Unsafety Sources"));
		AttrSource unsafetySources = town.getUnsafety();
		for (String source : unsafetySources.sources.keySet()) {
			Double value = unsafetySources.sources.get(source);
			out.add(CivColor.Green+source+": "+CivColor.LightGreen+value);
		}
		out.add(CivColor.LightPurple+"Total: "+CivColor.LightGreen+df.format(unsafetySources.total));

		out.add(CivMessage.buildSmallTitle("Total"));
		ConfigSafetyState state = town.getSafetyState();
		out.add(CivColor.LightGreen+df.format(town.getSafetyPercentage()*100)+"%"+CivColor.Green+" Safety. State: "+CivColor.valueOf(state.color)+state.name);
		CivMessage.send(sender, out);
	}
	...
	public static void show(CommandSender sender, Resident resident, Town town, Civilization civ, CommandBase parent) throws CivException {
		...
		if (resident == null || civ.hasResident(resident) || isAdmin) {
			...
			CivMessage.send(sender, CivColor.Green+"Claimed Plots: "+CivColor.LightGreen+"("+town.getTownChunks().size()+"/"+town.getMaxPlots()+")  "+
			...
			CivMessage.send(sender, CivColor.Green+"Total Members: "+CivColor.LightGreen+town.getResidentCount()+"  "+
									CivColor.Green+"Members Online: "+CivColor.LightGreen+town.getOnlineResidents().size());
			CivMessage.send(sender, CivColor.Green+"Property Tax Rate: "+CivColor.LightGreen+town.getTaxRateString()+"  "+
									CivColor.Green+"Player Tax Rate: "+CivColor.LightGreen+town.getFlatTax()+" Coins");
...
//					CivColor.LightGreen+df.format(town.getHappinessPercentage()*100)+"%");
			info.put("Happiness Percent", CivColor.LightGreen+df.format(Math.floor(town.getHappinessPercentage()*100))+"%");
			ConfigHappinessState state = town.getHappinessState();
			info.put("Happiness State", ""+CivColor.valueOf(state.color)+state.name);
			...
			info1.put("Safety Percent", CivColor.LightGreen+df.format(Math.floor(town.getSafetyPercentage()*100))+"%");
			ConfigSafetyState state1 = town.getSafetyState();
			info1.put("Safety State", ""+CivColor.valueOf(state1.color)+state1.name);
			CivMessage.send(sender, parent.makeInfoString(info1, CivColor.Green, CivColor.LightGreen));
			
			
			ConfigCultureLevel clc = CivSettings.cultureLevels.get(town.getCultureLevel());	
			CivMessage.send(sender, CivColor.Green+"Culture Level: "+clc.level+" ("+town.getAccumulatedCulture()+"/"+clc.amount+")");
		}
		...
		if (resident == null || town.isInGroup("mayors", resident) || town.isInGroup("assistants", resident) || 
			...
			try {
				CivMessage.send(sender, CivColor.Green+"Treasury: "+CivColor.LightGreen+town.getBalance()+" Coins  "+CivColor.Green+"Upkeep: "+CivColor.LightGreen+town.getTotalUpkeep()*town.getGovernment().upkeep_rate);
				...
				e.printStackTrace();
				throw new CivException("Internal Configuration Error in civcraft/command/town/TownInfoCommand.class");
			}
		}
		
		if (town.inDebt()) {
			CivMessage.send(sender, CivColor.Green+"Debt: "+CivColor.Yellow+town.getDebt()+" coins");
			CivMessage.send(sender, CivColor.Yellow+town.getDaysLeftWarning());
		}
