...
public class CookingPotComponent extends Component {

	/* What item it turns into */
	private LinkedList<CookingPotItem> items = new LinkedList<CookingPotItem>();
	...
	public void addRawFood(int source_type, short source_data, double rate, int result_type, short result_data, int amount) {
		CookingPotItem cpi = new CookingPotItem();
		cpi.source_type = source_type;
		cpi.source_data = source_data;
		cpi.rate = rate*1000;
		cpi.result_type = result_type;
		cpi.result_data = result_data;
		cpi.amount = amount;
		items.add(cpi);
	}
	...
	private void process(MultiInventory source, MultiInventory dest, int count, CivAsyncTask task) {
		...
		for (ItemStack stack : source.getContents()) {
			...
			CookingPotItem lowestChanceItem = null;
			boolean found = false;
			
			for (CookingPotItem si : items) {
				if (si == null) { 
					continue;
				}
