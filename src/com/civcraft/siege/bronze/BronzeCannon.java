...
public class BronzeCannon extends Buildable {

	public static HashMap<BlockCoord, BronzeCannon> fireSignLocations = new HashMap<BlockCoord, BronzeCannon>(); 
	public static HashMap<BlockCoord, BronzeCannon> angleSignLocations = new HashMap<BlockCoord, BronzeCannon>(); 
	public static HashMap<BlockCoord, BronzeCannon> powerSignLocations = new HashMap<BlockCoord, BronzeCannon>(); 
	public static HashMap<BlockCoord, BronzeCannon> cannonBlocks = new HashMap<BlockCoord, BronzeCannon>();
	...
	static {
		try {
			tntCost = CivSettings.getInteger(CivSettings.cannonConfig, "bronze_cannon.tnt_cost");
			maxCooldown = CivSettings.getInteger(CivSettings.cannonConfig, "bronze_cannon.cooldown");
			maxHitpoints = CivSettings.getInteger(CivSettings.cannonConfig, "bronze_cannon.hitpoints");
			baseStructureDamage = CivSettings.getInteger(CivSettings.cannonConfig, "bronze_cannon.structure_damage");
	...
	public static void newCannon(Resident resident) throws CivException {
		...
		BronzeCannon cannon = new BronzeCannon();
	...
	private static void removeAllValues(BronzeCannon cannon, HashMap<BlockCoord, BronzeCannon> map) {
		LinkedList<BlockCoord> removeUs = new LinkedList<BlockCoord>();
		for (BlockCoord bcoord : map.keySet()) {
			BronzeCannon c = map.get(bcoord);
			if (c == cannon) {
				removeUs.add(bcoord);
			}
		}
	...
	public void buildCannon(Player player, Location center) throws CivException {
		String templateFile;
		try {
			templateFile = CivSettings.getString(CivSettings.cannonConfig, "bronze_cannon.template");
		} catch (InvalidConfiguration e) {
			e.printStackTrace();
			return;
		}
		...
		for (int x = 0; x < regionX; x++) {
			for (int y = 0; y < regionY; y++) {
				for (int z = 0; z < regionZ; z++) {
					...
					if (BronzeCannon.cannonBlocks.containsKey(coord)) {
	...
	private void processCommandSigns(Template tpl, BlockCoord corner) {
		for (BlockCoord relativeCoord : tpl.commandBlockRelativeLocations) {
			...
			case "/fire":
				...
				BronzeCannon.fireSignLocations.put(coord, this);
				break;
			case "/angle":
				coord = new BlockCoord(absCoord);
				...
				BronzeCannon.angleSignLocations.put(coord, this);
				break;
			case "/power":
				coord = new BlockCoord(absCoord);
				...
				BronzeCannon.powerSignLocations.put(coord, this);
				break;
			case "/cannon":
				coord = new BlockCoord(absCoord);
				...
				for (int z = 0; z < tpl.size_z; z++) {
					...
					try {
						if (ItemManager.getId(nextBlock) != tpl.blocks[x][y][z].getType()) {
							...
							WarRegen.saveBlock(nextBlock, BronzeCannon.RESTORE_NAME, false);
	...
	public void processFire(PlayerInteractEvent event) throws CivException {
		...
		if (this.tntLoaded < tntCost) {
			...
			BronzeCannonProjectile proj = new BronzeCannonProjectile(this, cannonLocation.clone(), resident);
			proj.fire();
			this.tntLoaded = 0;
			...
			class SyncTask implements Runnable {
				...
				public SyncTask (BronzeCannon cannon) {
					this.cannon = cannon;
				}
