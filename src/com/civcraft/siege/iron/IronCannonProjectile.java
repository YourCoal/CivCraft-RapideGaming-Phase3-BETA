...
public class IronCannonProjectile {
	public IronCannon cannon;
	public Location loc;
	...
	static {
		try {
			yield = CivSettings.getDouble(CivSettings.cannonConfig, "iron_cannon.yield");
			playerDamage = CivSettings.getDouble(CivSettings.cannonConfig, "iron_cannon.player_damage");
			maxRange = CivSettings.getDouble(CivSettings.cannonConfig, "iron_cannon.max_range");
		} catch (InvalidConfiguration e) {
			e.printStackTrace();
		}
	}
	
	public IronCannonProjectile(IronCannon cannon, Location loc, Resident whoFired) {
		this.cannon = cannon;
		this.loc = loc;
	...
	private void explodeBlock(Block b) {
		WarRegen.saveBlock(b, IronCannon.RESTORE_NAME, false);
	...
	public void fire() {
		class SyncTask implements Runnable {
			IronCannonProjectile proj;
			
			public SyncTask(IronCannonProjectile proj) {
				this.proj = proj;
			}
