package com.civcraft.siege.steel;
...
public class SteelCannon extends Buildable {

	public static HashMap<BlockCoord, SteelCannon> fireSignLocations = new HashMap<BlockCoord, SteelCannon>(); 
	public static HashMap<BlockCoord, SteelCannon> angleSignLocations = new HashMap<BlockCoord, SteelCannon>(); 
	public static HashMap<BlockCoord, SteelCannon> powerSignLocations = new HashMap<BlockCoord, SteelCannon>(); 
	public static HashMap<BlockCoord, SteelCannon> cannonBlocks = new HashMap<BlockCoord, SteelCannon>();
	...
	static {
		try {
			tntCost = CivSettings.getInteger(CivSettings.cannonConfig, "steel_cannon.tnt_cost");
			maxCooldown = CivSettings.getInteger(CivSettings.cannonConfig, "steel_cannon.cooldown");
			maxHitpoints = CivSettings.getInteger(CivSettings.cannonConfig, "steel_cannon.hitpoints");
			baseStructureDamage = CivSettings.getInteger(CivSettings.cannonConfig, "steel_cannon.structure_damage");
	...
	public static void newCannon(Resident resident) throws CivException {
		...
		SteelCannon cannon = new SteelCannon();
	...
	private static void removeAllValues(SteelCannon cannon, HashMap<BlockCoord, SteelCannon> map) {
		LinkedList<BlockCoord> removeUs = new LinkedList<BlockCoord>();
		for (BlockCoord bcoord : map.keySet()) {
			SteelCannon c = map.get(bcoord);
			if (c == cannon) {
				removeUs.add(bcoord);
			}
		}
	...
	public void buildCannon(Player player, Location center) throws CivException {
		String templateFile;
		try {
			templateFile = CivSettings.getString(CivSettings.cannonConfig, "steel_cannon.template");
		} catch (InvalidConfiguration e) {
			e.printStackTrace();
			return;
		}
		...
		for (int x = 0; x < regionX; x++) {
			for (int y = 0; y < regionY; y++) {
				for (int z = 0; z < regionZ; z++) {
					...
					if (SteelCannon.cannonBlocks.containsKey(coord)) {
	...
	private void processCommandSigns(Template tpl, BlockCoord corner) {
		for (BlockCoord relativeCoord : tpl.commandBlockRelativeLocations) {
			...
			case "/fire":
				...
				SteelCannon.fireSignLocations.put(coord, this);
				break;
			case "/angle":
				coord = new BlockCoord(absCoord);
				...
				SteelCannon.angleSignLocations.put(coord, this);
				break;
			case "/power":
				coord = new BlockCoord(absCoord);
				...
				SteelCannon.powerSignLocations.put(coord, this);
				break;
			case "/cannon":
				coord = new BlockCoord(absCoord);
				...
				for (int z = 0; z < tpl.size_z; z++) {
					...
					try {
						if (ItemManager.getId(nextBlock) != tpl.blocks[x][y][z].getType()) {
							...
							WarRegen.saveBlock(nextBlock, SteelCannon.RESTORE_NAME, false);
	...
	public void processFire(PlayerInteractEvent event) throws CivException {
		...
		if (this.tntLoaded < tntCost) {
			...
			SteelCannonProjectile proj = new SteelCannonProjectile(this, cannonLocation.clone(), resident);
			proj.fire();
			this.tntLoaded = 0;
			...
			class SyncTask implements Runnable {
				...
				public SyncTask (SteelCannon cannon) {
					this.cannon = cannon;
				}
