...
public class SteelCannonProjectile {
	public SteelCannon cannon;
	public Location loc;
	...
	static {
		try {
			yield = CivSettings.getDouble(CivSettings.cannonConfig, "steel_cannon.yield");
			playerDamage = CivSettings.getDouble(CivSettings.cannonConfig, "steel_cannon.player_damage");
			maxRange = CivSettings.getDouble(CivSettings.cannonConfig, "steel_cannon.max_range");
		} catch (InvalidConfiguration e) {
			e.printStackTrace();
		}
	}
	
	public SteelCannonProjectile(SteelCannon cannon, Location loc, Resident whoFired) {
		this.cannon = cannon;
		this.loc = loc;
	...
	private void explodeBlock(Block b) {
		WarRegen.saveBlock(b, SteelCannon.RESTORE_NAME, false);
					...
					if (loc.distance(b.getLocation()) <= yield) {
						...
						if (sb != null) {
							...
							if (!sb.getOwner().isDestroyed()) {
								...
								if (!SteelCannon.cannonBlocks.containsKey(bcoord)) {
									explodeBlock(b);
								}
							}
							continue;
	...
	public void fire() {
		class SyncTask implements Runnable {
			SteelCannonProjectile proj;
			
			public SyncTask(SteelCannonProjectile proj) {
				this.proj = proj;
			}
