...
public class TitaniumCannon extends Buildable {

	public static HashMap<BlockCoord, TitaniumCannon> fireSignLocations = new HashMap<BlockCoord, TitaniumCannon>(); 
	public static HashMap<BlockCoord, TitaniumCannon> angleSignLocations = new HashMap<BlockCoord, TitaniumCannon>(); 
	public static HashMap<BlockCoord, TitaniumCannon> powerSignLocations = new HashMap<BlockCoord, TitaniumCannon>(); 
	public static HashMap<BlockCoord, TitaniumCannon> cannonBlocks = new HashMap<BlockCoord, TitaniumCannon>();
	...
	static {
		try {
			tntCost = CivSettings.getInteger(CivSettings.cannonConfig, "titanium_cannon.tnt_cost");
			maxCooldown = CivSettings.getInteger(CivSettings.cannonConfig, "titanium_cannon.cooldown");
			maxHitpoints = CivSettings.getInteger(CivSettings.cannonConfig, "titanium_cannon.hitpoints");
			baseStructureDamage = CivSettings.getInteger(CivSettings.cannonConfig, "titanium_cannon.structure_damage");
	...
	public static void newCannon(Resident resident) throws CivException {
		...
		TitaniumCannon cannon = new TitaniumCannon();
	...
	private static void removeAllValues(TitaniumCannon cannon, HashMap<BlockCoord, TitaniumCannon> map) {
		LinkedList<BlockCoord> removeUs = new LinkedList<BlockCoord>();
		for (BlockCoord bcoord : map.keySet()) {
			TitaniumCannon c = map.get(bcoord);
			if (c == cannon) {
				removeUs.add(bcoord);
			}
		}
	...
	public void buildCannon(Player player, Location center) throws CivException {
		String templateFile;
		try {
			templateFile = CivSettings.getString(CivSettings.cannonConfig, "titanium_cannon.template");
		} catch (InvalidConfiguration e) {
			e.printStackTrace();
			return;
		}
		...
		for (int x = 0; x < regionX; x++) {
			for (int y = 0; y < regionY; y++) {
				for (int z = 0; z < regionZ; z++) {
					...
					if (TitaniumCannon.cannonBlocks.containsKey(coord)) {
	...
	private void processCommandSigns(Template tpl, BlockCoord corner) {
		for (BlockCoord relativeCoord : tpl.commandBlockRelativeLocations) {
			...
			case "/fire":
				...
				TitaniumCannon.fireSignLocations.put(coord, this);
				break;
			case "/angle":
				coord = new BlockCoord(absCoord);
				...
				TitaniumCannon.angleSignLocations.put(coord, this);
				break;
			case "/power":
				coord = new BlockCoord(absCoord);
				...
				TitaniumCannon.powerSignLocations.put(coord, this);
				break;
			case "/cannon":
				coord = new BlockCoord(absCoord);
				...
				for (int z = 0; z < tpl.size_z; z++) {
					...
					try {
						if (ItemManager.getId(nextBlock) != tpl.blocks[x][y][z].getType()) {
							...
							WarRegen.saveBlock(nextBlock, TitaniumCannon.RESTORE_NAME, false);
	...
	public void processFire(PlayerInteractEvent event) throws CivException {
		...
		if (this.tntLoaded < tntCost) {
			...
			TitaniumCannonProjectile proj = new TitaniumCannonProjectile(this, cannonLocation.clone(), resident);
			proj.fire();
			this.tntLoaded = 0;
			this.shotCooldown = maxCooldown;
			
			class SyncTask implements Runnable {
				TitaniumCannon cannon;
				
				public SyncTask (TitaniumCannon cannon) {
					this.cannon = cannon;
				}
