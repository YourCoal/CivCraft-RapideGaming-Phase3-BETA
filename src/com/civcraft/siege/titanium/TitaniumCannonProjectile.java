...
public class TitaniumCannonProjectile {
	public TitaniumCannon cannon;
	public Location loc;
	...
	static {
		try {
			yield = CivSettings.getDouble(CivSettings.cannonConfig, "titanium_cannon.yield");
			playerDamage = CivSettings.getDouble(CivSettings.cannonConfig, "titanium_cannon.player_damage");
			maxRange = CivSettings.getDouble(CivSettings.cannonConfig, "titanium_cannon.max_range");
		} catch (InvalidConfiguration e) {
			e.printStackTrace();
		}
	}
	
	public TitaniumCannonProjectile(TitaniumCannon cannon, Location loc, Resident whoFired) {
		this.cannon = cannon;
		this.loc = loc;
	...
	private void explodeBlock(Block b) {
		WarRegen.saveBlock(b, TitaniumCannon.RESTORE_NAME, false);
					...
					if (loc.distance(b.getLocation()) <= yield) {
						...
						if (sb != null) {
							...
							if (!sb.getOwner().isDestroyed()) {
								...
								if (!TitaniumCannon.cannonBlocks.containsKey(bcoord)) {
									explodeBlock(b);
								}
							}
							continue;
	...
	public void fire() {
		class SyncTask implements Runnable {
			TitaniumCannonProjectile proj;
			
			public SyncTask(TitaniumCannonProjectile proj) {
				this.proj = proj;
			}
